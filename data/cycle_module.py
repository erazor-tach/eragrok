    ...
    if preset == "Premier Cycle":
        ...
        for cid, lb in getattr(app, "cycle_category_listboxes", {}).items():
            try:
                ...
                pass
        # Pré-remplir doses
        app.root.after(100, lambda: _preset_premier_cycle_doses(app))
...
def show_cycle_disclaimer(app):
    ...
    if user:
        flag_file = os.path.join(utils.USERS_DIR, user, "cycle_qualified.ok")
...
def _refresh(app):
    ...
    try:
        ...
        for row in reversed(data_rows[-10:]):
            while len(row) < 5:
                row.append("")
            app.cycle_tree.insert("", tk.END, values=(row[0], row[1], row[2], row[3], row[4]))
    except Exception:
        pass
...
def _save(app):
    ...
    try:
        ...
        header = ["Début", "Fin estimée", "Longueur (sem)", "Produits & doses", "Note"]
        if not os.path.exists(fichier):
            with open(fichier, "w", newline="", encoding="utf-8") as f:
                csv.writer(f).writerow(header)

        with open(fichier, "a", newline="", encoding="utf-8") as f:
            csv.writer(f).writerow([start_date, fin_text, length_val, produits_field, note])
...
def _rebuild_dose_fields(app):
    ...
    for prod in selected:
        ...
        if not is_oral:
            ...
            app.root.after(50, _make_vial_updater())
        else:
    ...
    # Relancer le résumé
    app.root.after(60, lambda: _update_advice_summary(app))
