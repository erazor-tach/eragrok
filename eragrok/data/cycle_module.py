...
def _apply_cycle_preset(app, preset):
    ...
    if preset == "Premier Cycle":
        # Pré-sélectionner : Testosterone Enanthate (cat0) + HCG (cat4) + Anastrozole + Clomid/Nolvadex
        for cid, lb in getattr(app, "cycle_category_listboxes", {}).items():
            try:
                ...
                if target and target in items:
                    ...
                    # Mettre à jour le compteur sélection
                    for w in lb.master.master.winfo_children():
                        try:
                            if "sélectionné" in w.cget("text"):
                                w.configure(text=f"{len(lb.curselection())} sélectionné(s)")
                        except Exception:
                            pass
            except Exception:
                pass
        # lb.selection_set() ne déclenche pas <<ListboxSelect>> — forcer rebuild
        _rebuild_dose_fields(app)
        # Pré-remplir doses après création des champs
        def _safe_preset():
            try:
                ...
                pass
        try:
            app.root.after(120, _safe_preset)
        except Exception:
            _safe_preset()
...
def _save(app):
    ...
    try:
        ...
        # Clés composées PCT (ex: "Clomiphene (Clomid)_J+14") → stockées séparément
        # Clés de produits normaux → stockées dans produits_doses
        _PCT_BASE = {"Clomiphene (Clomid)", "Tamoxifen (Nolvadex)"}
        produits_doses = []
        pct_protocol   = []   # clés J+xx stockées dans un champ séparé
        for prod, entry in getattr(app, "cycle_product_doses", {}).items():
            # Sauter le bare entry caché des produits PCT
            if prod in _PCT_BASE:
                continue
            try:
                dose = entry.get().strip()
            except Exception:
                dose = ""
            # Clés composées PCT  ex: "Clomiphene (Clomid)_J+14"
            if "_J+" in prod:
                if dose:
                    pct_protocol.append(f"{prod}: {dose}")
                continue
            if dose:
                produits_doses.append(f"{prod}: {dose}")
            else:
                produits_doses.append(prod)
        # Ajouter le protocole PCT en fin, préfixé [PCT]
        for p in pct_protocol:
            produits_doses.append(f"[PCT]{p}")
        ...
        messagebox.showinfo("ERAGROK",
            f"✔  Cycle du {start_date} sauvegardé dans l'historique.")
...
def _rebuild_dose_fields(app):
    ...
    for prod in selected:
        ...
        # ── Carte produit ─────────────────────────────────────────────────────
        card = ctk.CTkFrame(frame, fg_color=TH.BG_CARD2, corner_radius=10)
        card.pack(fill=tk.X, padx=12, pady=5)

        # Ligne 1 — Nom + métadonnées
        hdr = ctk.CTkFrame(card, fg_color=TH.BG_INPUT, corner_radius=0)
        ...
        # Valeur par défaut selon le produit
        _DEFAULTS = {
            "Anastrozole (Arimidex)": "0.25",
        }
        _default_dose = old_doses.get(prod, "") or _DEFAULTS.get(prod, "")
        dose_var = tk.StringVar(value=_default_dose)
        _placeholder = {
            "Anastrozole (Arimidex)": "0.25",
            "Clomiphene (Clomid)":    "50 mg/j",
            "Tamoxifen (Nolvadex)":   "20 mg/j",
        }.get(prod, "ex: 500")
        ...
        _is_pct_prod = prod in {"Clomiphene (Clomid)", "Tamoxifen (Nolvadex)"}
        if _is_pct_prod:
            dose_row.pack_forget()  # Doses PCT gérées par protocole ci-dessous
        else:
            dose_entry.pack(side=tk.LEFT, padx=(4, 4))
            _unit = "mg EOD" if prod == "Anastrozole (Arimidex)" else "/ semaine"
            ctk.CTkLabel(dose_row, text=_unit, font=("Inter", 10),
        ...
        # Note spéciale Anastrozole
        if prod == "Anastrozole (Arimidex)":
            note_ai = ctk.CTkFrame(card, fg_color=TH.ACCENT_DIM, corner_radius=6)
            note_ai.pack(fill=tk.X, padx=12, pady=(2, 4))
            ctk.CTkLabel(note_ai,
                text="  ℹ️  Départ conseillé : 0.25 mg EOD (tous les 2 jours). "
                     "Adapter selon bilan sanguin : augmenter si E2 élevé, "
                     "réduire ou supprimer si E2 trop bas (symptômes : douleurs articulaires, libido ↓).",
                font=("Inter", 9), text_color=TH.ACCENT_GLOW,
        ...
        if not is_oral:
            ...
            ctk.CTkLabel(vial_row, text="Vol.", font=("Inter", 10),
                         text_color="#777777").pack(side=tk.LEFT)
            vol_var = tk.StringVar(value=old_vol.get(prod, "") or "10")
            ...
            _is_pct = prod in {"Clomiphene (Clomid)", "Tamoxifen (Nolvadex)"}

            if _is_pct:
                # ── Bloc PCT : protocole en 2 phases ──────────────────────────
                try:
                    pct_mode = getattr(app, "cycle_pct_mode_var", None)
                    pct_mode = pct_mode.get() if pct_mode else "Normal"
                except Exception:
                    pct_mode = "Normal"

                _PCT_SCHEDULES = {
                    "Clomiphene (Clomid)": {
                        "Normal":   [("J+14", "50 mg/j", "50"),
                                     ("J+28", "25 mg/j", "25")],
                        "Agressif": [("J+14", "100 mg/j → 50 mg après J+10", "100"),
                                     ("J+28", "50 mg/j", "50"),
                                     ("J+42", "25 mg/j", "25")],
                    },
                    "Tamoxifen (Nolvadex)": {
                        "Normal":   [("J+14", "20 mg/j", "20"),
                                     ("J+28", "20 mg/j", "20")],
                        "Agressif": [("J+14", "40 mg/j", "40"),
                                     ("J+28", "20 mg/j", "20"),
                                     ("J+42", "10 mg/j", "10")],
                    },
                }
                ...
                # Vérifier compatibilité PCT avec fin de cycle
                try:
                    end_mode = getattr(app, "cycle_end_var", None)
                    end_mode = end_mode.get() if end_mode else "PCT"
                except Exception:
                    end_mode = "PCT"

                if end_mode in ("TRT", "Cruise"):
                    incompat_f = ctk.CTkFrame(card, fg_color="#1a1020", corner_radius=8)
                    incompat_f.pack(fill=tk.X, padx=12, pady=(4, 8))
                    ctk.CTkLabel(incompat_f,
                        text=f"  ⚠️  Incompatible avec {end_mode}  —  "
                             f"Le PCT n'est pas utilisé en {end_mode}. "
                             f"La testostérone exogène maintient l'axe supprimé.",
                        font=("Inter", 9), text_color=TH.WARNING,
                        wraplength=820, justify="left", anchor="w",
                    ).pack(anchor="w", padx=10, pady=6)
                else:
                    pct_info = ctk.CTkFrame(card, fg_color=TH.BG_INPUT, corner_radius=0)
                    pct_info.pack(fill=tk.X, padx=0, pady=(4, 0))
                    ctk.CTkLabel(pct_info,
                        text=f"  📋  Protocole PCT — {pct_mode}  "
                             f"(commence 14 jours après la dernière injection)",
                        font=("Inter", 9, "bold"), text_color=TH.ACCENT_GLOW,
                    ).pack(anchor="w", padx=10, pady=(5, 2))

                for phase_lbl, default_dose_txt, default_val in (schedule if end_mode not in ("TRT","Cruise") else []):
                    phase_row = ctk.CTkFrame(card, fg_color="transparent")
                    phase_row.pack(fill=tk.X, padx=12, pady=(2, 0))

                    ctk.CTkLabel(phase_row,
                        text=phase_lbl, font=("Inter", 10, "bold"),
                        text_color=TH.PURPLE if "42" in phase_lbl else
                                   TH.ACCENT_GLOW if "14" in phase_lbl else TH.TEXT,
                        width=55,
                    ).pack(side=tk.LEFT)

                    ctk.CTkLabel(phase_row,
                        text=default_dose_txt,
                        font=("Inter", 10), text_color=TH.TEXT_SUB,
                        width=200,
                    ).pack(side=tk.LEFT, padx=(4, 12))

                    pct_dose_key = f"{prod}_{phase_lbl}"
                    old_pct_val = old_doses.get(pct_dose_key, "") or default_val
                    pct_dose_var = tk.StringVar(value=old_pct_val)
                    pct_entry = ctk.CTkEntry(
                        phase_row, textvariable=pct_dose_var,
                        width=80, height=28,
                        placeholder_text=default_val,
                        font=("Inter", 10),
                    )
                    pct_entry.pack(side=tk.LEFT, padx=(0, 4))
                    ctk.CTkLabel(phase_row, text="mg/j",
                        font=("Inter", 9), text_color="#666666",
                    ).pack(side=tk.LEFT)
                    # Stocker avec clé composée pour la sauvegarde
                    app.cycle_product_doses[pct_dose_key] = pct_entry
...
def show_cycle_screen(app):
    ...
    # ── BLOC 2 : Données du cycle (compact, 1 ligne) ─────────────────────────
    data_card = mk_card(scroll)
    data_card.pack(**PAD)
    mk_title(data_card, "  💊  DONNÉES DU CYCLE").pack(anchor=tk.W, padx=16, pady=(14, 6))
    mk_sep(data_card).pack(fill=tk.X, padx=16, pady=(0, 10))

    # ── Ligne 1 : Preset · Longueur · Fin de cycle · Mode PCT ─────────────
    row1 = ctk.CTkFrame(data_card, fg_color="transparent", height=38)
    row1.pack(fill=tk.X, padx=16, pady=(0, 6))
    row1.pack_propagate(False)

    # Preset
    mk_label(row1, "Type", size="small", color=TH.TEXT_SUB).pack(side=tk.LEFT, padx=(0,4))
    app.cycle_preset_var = tk.StringVar(value="12 semaines")
    presets = ["12 semaines", "16 semaines", "Personnalisé", "Premier Cycle"]
    try:
        preset_menu = ctk.CTkOptionMenu(
            row1, values=presets, variable=app.cycle_preset_var, width=140,
            command=lambda v: (_apply_cycle_preset(app, v), _update_fin_date(app)),
        )
    except Exception:
        preset_menu = tk.OptionMenu(
            row1, app.cycle_preset_var, *presets,
            command=lambda v: (_apply_cycle_preset(app, v), _update_fin_date(app)),
        )
    preset_menu.pack(side=tk.LEFT, padx=(0,16))

    # Longueur
    mk_label(row1, "Semaines", size="small", color=TH.TEXT_MUTED).pack(side=tk.LEFT, padx=(0,4))
    app.cycle_length_var = tk.StringVar(value="12")
    length_entry = mk_entry(row1, width=60, placeholder="sem")
    try: length_entry.configure(textvariable=app.cycle_length_var)
    except: length_entry.insert(0, app.cycle_length_var.get())
    ...
    ctk.CTkFrame(row1, fg_color=TH.BORDER, width=1, height=28).pack(side=tk.LEFT, padx=(0,16))

    # Fin de cycle
    mk_label(row1, "Fin :", size="small", color=TH.TEXT_SUB).pack(side=tk.LEFT, padx=(0,6))
    app.cycle_end_var = tk.StringVar(value="PCT")
    for opt, color in [("PCT", TH.ACCENT), ("Cruise", "#40c0e0"), ("TRT", "#c080ff")]:
        ctk.CTkRadioButton(
            row1, text=opt, variable=app.cycle_end_var, value=opt,
            font=TH.F_SMALL, text_color=TH.TEXT,
            fg_color=color, hover_color=TH.ACCENT_DIM, width=10,
        ).pack(side=tk.LEFT, padx=(0,12))

    ctk.CTkFrame(row1, fg_color=TH.BORDER, width=1, height=28).pack(side=tk.LEFT, padx=(0,16))

    # Mode PCT
    mk_label(row1, "PCT :", size="small", color=TH.TEXT_SUB).pack(side=tk.LEFT, padx=(0,6))
    app.cycle_pct_mode_var = tk.StringVar(value="Normal")
    for mode, col in [("Normal", TH.ACCENT), ("Agressif", TH.WARNING)]:
        ctk.CTkRadioButton(
            row1, text=mode, variable=app.cycle_pct_mode_var, value=mode,
            font=TH.F_SMALL, text_color=TH.TEXT,
            fg_color=col, hover_color=TH.ACCENT_DIM, width=10,
        ).pack(side=tk.LEFT, padx=(0,12))

    # ── Ligne 2 : Date début · Fin estimée · Dose maintenance ─────────────
    row2 = ctk.CTkFrame(data_card, fg_color="transparent", height=36)
    row2.pack(fill=tk.X, padx=16, pady=(0, 4))
    row2.pack_propagate(False)

    mk_label(row2, "Début :", size="small", color=TH.TEXT_SUB).pack(side=tk.LEFT, padx=(0,4))
    app.cycle_start_var = tk.StringVar(value="")
    start_entry = mk_entry(row2, width=110, placeholder="JJ/MM/AAAA")
    try: start_entry.configure(textvariable=app.cycle_start_var, state="readonly", cursor="hand2")
    except: pass
    start_entry.pack(side=tk.LEFT, padx=(0,4))
    start_entry.bind("<Button-1>", lambda e: _pick_start_date(app))
    _btn_safe(row2, "📅", lambda: _pick_start_date(app), "ACCENT", width=32, height=28).pack(side=tk.LEFT, padx=(0,20))

    mk_label(row2, "Fin estimée :", size="small", color=TH.TEXT_MUTED).pack(side=tk.LEFT, padx=(0,6))
    app.cycle_fin_label = ctk.CTkLabel(row2, text="—", font=TH.F_SMALL, text_color=TH.ACCENT)
    app.cycle_fin_label.pack(side=tk.LEFT, padx=(0,20))

    # Dose maintenance TRT/Cruise (dynamique inline)
    app.cycle_maintenance_dose_var = tk.StringVar(value="")
    maintenance_frame = ctk.CTkFrame(row2, fg_color="transparent")
    maintenance_frame.pack(side=tk.LEFT)

    # Frame bannière incompatibilité PCT
    pct_warning_frame = ctk.CTkFrame(data_card, fg_color="transparent")
    pct_warning_frame.pack(fill=tk.X, padx=16, pady=(0, 4))

    def _update_pct_warning(*_):
        for w in pct_warning_frame.winfo_children():
            w.destroy()
        mode = app.cycle_end_var.get()
        if mode not in ("TRT", "Cruise"):
            return
        pct_names = {"Clomiphene (Clomid)", "Tamoxifen (Nolvadex)"}
        selected_prods = set(_gather_selected_products(app))
        has_pct = bool(selected_prods & pct_names)
        if not has_pct:
            return
        warn_f = ctk.CTkFrame(pct_warning_frame, fg_color="#1a0a00", corner_radius=6)
        warn_f.pack(fill=tk.X)
        msg = (
            f"  ⚠️  Incompatible : {mode} + PCT (Clomid / Nolvadex) — "
            f"En {mode}, la testostérone exogène maintient l'axe HPTA supprimé. "
            f"Le PCT sert à relancer l'axe après arrêt total. Retire Clomid / Nolvadex."
        )
        ctk.CTkLabel(warn_f, text=msg, font=("Inter", 9), text_color=TH.WARNING,
                     wraplength=900, justify="left", anchor="w",
        ).pack(anchor="w", padx=12, pady=5)

    app.cycle_end_var.trace_add("write", _update_pct_warning)
    app._pct_warning_updater = _update_pct_warning
    ...
    def _update_maintenance_visibility(*_):
        ...
        if mode in ("TRT", "Cruise"):
            ...
            mk_label(maintenance_frame, f"Dose {mode} :", size="small",
                     color=TH.TEXT_SUB).pack(side=tk.LEFT, padx=(0,4))
            e = mk_entry(maintenance_frame, width=80, placeholder="mg/sem")
            try: e.configure(textvariable=app.cycle_maintenance_dose_var)
            except: pass
            e.pack(side=tk.LEFT, padx=(0,4))
            ctk.CTkLabel(maintenance_frame, text="mg/sem", font=("Inter", 10),
    ...
    ctk.CTkFrame(data_card, fg_color="transparent", height=4).pack()


    # ── BLOC 3 : Doses dynamiques par produit ─────────────────────────────────
    ...
    if _feat:
        _btn_safe(btn_row, "⏱  Calculateur demi-vie",
                  lambda: _feat.show_halflife_calculator(app),
                  "ACCENT", width=200, height=TH.BTN_LG).pack(side=tk.LEFT, padx=(0, 10))
    ...
    cols_t = ("Début", "Fin estimée", "Sem.", "Produits & doses", "Note")
    tree_wrap = ctk.CTkFrame(hist_card, fg_color="transparent")
    tree_wrap.pack(fill=tk.X, padx=16, pady=(0, 4))
    vsb = ttk.Scrollbar(tree_wrap, orient="vertical")
    hsb = ttk.Scrollbar(hist_card,  orient="horizontal")
    app.cycle_tree = ttk.Treeview(
        tree_wrap, columns=cols_t, show="headings",
        height=6, selectmode="browse", style="Cycle.Treeview",
        yscrollcommand=vsb.set,
    )
    hsb.configure(command=app.cycle_tree.xview)
    app.cycle_tree.configure(xscrollcommand=hsb.set)
    vsb.configure(command=app.cycle_tree.yview)
    col_w = {"Début": 100, "Fin estimée": 100, "Sem.": 60, "Produits & doses": 480, "Note": 200}
    for c in cols_t:
        app.cycle_tree.heading(c, text=c)
        anchor = tk.W if c in ("Produits & doses", "Note") else tk.CENTER
        app.cycle_tree.column(c, width=col_w.get(c, 120), anchor=anchor, stretch=(c == "Produits & doses"))
    app.cycle_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
    vsb.pack(side=tk.RIGHT, fill=tk.Y)
    hsb.pack(fill=tk.X, padx=16, pady=(0, 12))
