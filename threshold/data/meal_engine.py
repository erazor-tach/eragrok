...
# ══════════════════════════════════════════════════════════════════════════════
#  BUDGET PASS — port de _apply_budget_pass (features_module.py ERAGROK)
# ══════════════════════════════════════════════════════════════════════════════
...
def _apply_budget_pass(plan: list, selected: list, budget_day: float, app) -> list:
    ...
    Stratégie (identique ERAGROK) :
      1. Calculer coût estimé via prices_module
      2. Si coût <= budget → rien à faire
      3. Remplacer les items les plus chers par des alternatives de même macro
      4. Réduire la portion de 15% si aucune alternative dispo
      5. Maxi 6 passes pour converger
    """
    import copy

    try:
        from data.prices_module import get_prices
    except Exception:
    ...
    def _day_cost(p):
        ...
        for meal in p:
            for item in meal.get("items", []):
                food     = item.get("food", "")
                g_cooked = item.get("g", item.get("grams", 0))
                g_raw    = g_cooked * COOKED_TO_RAW.get(food, 1.0)
                pi       = prices.get(food)
                if not pi or not pi.get("prix_kg"):
                    continue
    ...
    # Index aliment → catégorie macro (utilise FOOD_CATS local)
    PROT_CATS = {"oeuf", "proteine_maigre", "proteine_grasse",
                 "laitier", "laitier_lent", "whey", "supplement_lent"}
    ...
    def _sorted_by_price(macro: str) -> list:
        ...
        def _px(f):
            ...
            return (pxkg * 0.6) if pi.get("unite") == "unite" else (pxkg / 10)
    ...
    def _item_cost_per_g(food: str) -> float:
        ...
        unite = pi.get("unite", "kg")
        if unite == "unite":
            return pxkg / (60 if "Oeuf" in food else 35)
        return pxkg / 1000
    ...
    for _pass in range(6):
        cost = _day_cost(plan)
        if cost <= budget_day * 1.05:
            break
        if cost - budget_day <= 0.10:
            break

        # Item le plus cher (hors slot coucher)
        most_expensive = None
        max_cost_item  = 0.0
        meal_idx = item_idx = -1
        ...
        if alternatives:
            ...
            if new_nutr:
                ...
                if old_nutr and old_nutr[1] > 0 and np > 0:
                    ...
                    new_grams = max(50, min(new_grams,
                                           FOOD_MAX_PORTION.get(new_food, FOOD_MAX_DEFAULT)))
                else:
                    new_grams = grams_exp
            ...
            # Réduire portion de 15%, supprimer si sous le minimum
