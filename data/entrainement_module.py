...
def _load_all_programmes(app):
    """Charge programmes.json → liste de dicts [{id, title, created, lines}, ...]."""
    path = _prog_file(app)
...
def _write_all_programmes(app, programmes):
    """
    Sauvegarde la liste dans programmes.json avec rotation de backups.
    Avant d'écrire, crée programmes.bak1 → .bak4 par rotation.
    """
    path = _prog_file(app)
    # Rotation backups : bak4←bak3←bak2←bak1←current
    for i in range(_PROG_BACKUPS, 0, -1):
        src_bak = path.replace(".json", f".bak{i-1}") if i > 1 else path
        dst_bak = path.replace(".json", f".bak{i}")
