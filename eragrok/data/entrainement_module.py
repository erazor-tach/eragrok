...
def show_entrainement_screen(app, program=None):
    ...
    def _bind_lb_scroll(lb):
        """Bind MouseWheel sur un tk.Listbox — nécessaire dans .exe PyInstaller."""
        def _sc(ev): lb.yview_scroll(int(-1*(ev.delta/120)), "units")
        def _sc_up(ev): lb.yview_scroll(-1, "units")
        def _sc_dn(ev): lb.yview_scroll(1,  "units")
        lb.bind("<MouseWheel>", _sc,    add="+")
        lb.bind("<Button-4>",   _sc_up, add="+")
        lb.bind("<Button-5>",   _sc_dn, add="+")
    ...
    # Scroll molette Treeview dans .exe PyInstaller
    def _tree_scroll(ev):
        app.tech_tree.yview_scroll(int(-1*(ev.delta/120)), "units")
    def _tree_scroll_up(ev):
        app.tech_tree.yview_scroll(-1, "units")
    def _tree_scroll_dn(ev):
        app.tech_tree.yview_scroll(1, "units")
    app.tech_tree.bind("<MouseWheel>", _tree_scroll,    add="+")
    app.tech_tree.bind("<Button-4>",   _tree_scroll_up, add="+")
    app.tech_tree.bind("<Button-5>",   _tree_scroll_dn, add="+")
    ...
    # Enveloppé dans mk_scrollframe → scroll molette fonctionne dans le .exe
    c2_outer = ctk.CTkFrame(main, fg_color="transparent")
    c2_outer.grid(row=0, column=2, sticky="nsew")
    c2 = mk_scrollframe(c2_outer)
    ...
    _bind_lb_scroll(app.schedule_lb)
    ...
    _bind_lb_scroll(app.program_lb)
    ...
    _bind_lb_scroll(app.saved_prog_lb)
