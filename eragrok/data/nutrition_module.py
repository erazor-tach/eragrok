...
def _delete_selected(app):
    ...
    dates = [app.nutrition_tree.item(s, "values")[0]
             for s in sel if app.nutrition_tree.item(s, "values")]
    try:
        for d in dates:
...
def render_dashboard(app):
    ...
    # Nettoyer les canvases matplotlib en cours (évite bgerror check_dpi_scaling)
    ...
    if _feat:
        ...
        if _plan_data:
            ...
            plan_scroll.configure(height=460)
...
def show_nutrition_screen(app):
    ...
    def _export_plan_pdf():
        try:
            ...
            from data import pdf_utils
            if last.get("days"):
                pdf_utils.export_multiday_plan_pdf(app, last["days"], ask_path=True)
            elif last.get("plan"):
    ...
    mk_btn(left, "📋  PDF Historique nutrition", _export_nutr_pdf,
           ...
           width=210, height=TH.BTN_SM).pack(padx=14, pady=(0, 6))
    mk_btn(left, "🍽  PDF Plan alimentaire", _export_plan_pdf,
           color="#1a1a2e", hover="#252540",
           width=210, height=TH.BTN_SM).pack(padx=14, pady=(0, 14))
    ...
    if _feat:
        ...
        if _plan_data:
            ...
            plan_scroll.pack(fill="x", padx=8, pady=(0,12))
            plan_scroll.configure(height=500)
            plan_inner = ctk.CTkFrame(plan_scroll, fg_color="transparent")
            plan_inner.pack(fill="both", expand=True)
            _days = last_plan.get("days") if last_plan else None
            _feat.render_plan_card(plan_inner, app, _plan_data,
    ...
    app.nutrition_tree.pack(fill="both", expand=True, padx=16, pady=(0, 4))
