...
def render_dashboard(app):
    ...
    if _feat:
        ...
        # Fallback : charger le dernier plan accepté depuis SQLite
        if not last_plan:
            try:
                db_plan = _db.meal_plan_get_last(app, accepted_only=True)
                if db_plan:
                    plan_data = db_plan["plan_json"]
                    is_multi  = db_plan["mode"] != "jour"
                    last_plan = {
                        "cal": db_plan["cal_target"], "prot": db_plan["prot_target"],
                        "gluc": db_plan["gluc_target"], "lip": db_plan["lip_target"],
                        "n_meals": db_plan["n_meals"], "adj": db_plan["adj_label"],
                        "accepted": db_plan["accepted"],
                        "budget_w": db_plan.get("budget_w", 0),
                        "multiday": is_multi,
                    }
                    if is_multi:
                        # Restaurer les dates depuis ISO strings
                        import datetime as _dt_restore
                        for d in plan_data:
                            if isinstance(d.get("date"), str):
                                try:
                                    d["date"] = _dt_restore.date.fromisoformat(d["date"])
                                except Exception:
                                    pass
                        last_plan["days"] = plan_data
                    else:
                        last_plan["plan"] = plan_data
                    app._last_meal_plan = last_plan
            except Exception:
                pass
