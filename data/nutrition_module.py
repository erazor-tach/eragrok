...
def _read_last_cycle(app):
    """
    Lit la dernière entrée de cycle.csv.
    Retourne un dict {debut, fin, longueur, produits, note} ou None.
    """
    if not getattr(app, "current_user", None):
        return None
    p = os.path.join(utils.USERS_DIR, app.current_user, "cycle.csv")
    ...
    try:
        ...
        if len(rows) < 2:
            return None
        row = rows[-1]   # dernière entrée
        return {
            "debut":    row[0] if len(row) > 0 else "—",
            "fin":      row[1] if len(row) > 1 else "—",
            "longueur": row[2] if len(row) > 2 else "—",
            "produits": row[3] if len(row) > 3 else "",
...
def _render_week_training(card, app, monday, sunday):
    ...
    fpath = os.path.join(utils.USERS_DIR, getattr(app, "current_user", ""), "planning.csv")
                ...
                for row in reader:
                    ...
                    if d and d in week_dates:
                        sessions.append({
                            ...
                            "types":  row[3].strip() if len(row) > 3 else "",
                            "line":   row[5].strip() if len(row) > 5 else "",
                        })
        except Exception:
            pass
    ...
    # Dédupliquer par (date, line)
    seen = set()
    for s in sessions:
        key = (s["date"], s["line"] or s["prog"])
        if key in seen:
            continue
        seen.add(key)
        ...
        # Ligne 1 : Jour + date + type programme
        l1 = ctk.CTkFrame(row_f, fg_color="transparent")
        l1.pack(fill="x", padx=10, pady=(5, 1))
        day_str = f"{DAY_FR[s['date'].weekday()]} {s['date']:%d/%m}"
        mk_label(l1, day_str, size="small",
                 color=TH.ACCENT_GLOW, width=65).pack(side="left")
        if s["prog"]:
            mk_badge(l1, s["prog"],
                     color=TH.ACCENT_DIM,
                     text_color=TH.ACCENT_GLOW).pack(side="left", padx=6)

        # Ligne 2 : Partie du corps / catégorie
        partie = s["groupes"] or s["types"]
        if partie:
            l2 = ctk.CTkFrame(row_f, fg_color="transparent")
            l2.pack(fill="x", padx=12, pady=(0, 1))
            mk_label(l2, f"💪 {partie}", size="small",
                     color=TH.TEXT_SUB).pack(side="left")

        # Ligne 3 : Technique complète (nom [reps] | charge)
        if s["line"]:
            # Nettoyer l'ID entre parenthèses en fin de ligne
            import re as _re
            tech_display = _re.sub(r"\s*\([A-Z0-9_]+\)\s*$", "", s["line"]).strip()
            l3 = ctk.CTkFrame(row_f, fg_color="transparent")
            l3.pack(fill="x", padx=12, pady=(0, 5))
            mk_label(l3, f"🔧 {tech_display}", size="small",
                     color=TH.TEXT_MUTED).pack(side="left")
        else:
...
def _render_week_nutrition(card, app, monday, sunday):
    ...
    import os, csv as _csv
    week_dates = set(_week_dates(monday, sunday))
    fpath = os.path.join(utils.USERS_DIR,
                         getattr(app, "current_user", ""), "nutrition.csv")
        ...
        try:
            with open(fpath, "r", newline="", encoding="utf-8") as f:
                ...
                for row in reader:
                    if not row:
                        continue
                    d = _parse_date_flex(row[0] if row else "")
                    if d and d in week_dates:
                        rows_week.append(row)
        except Exception:
            pass
...
def _render_week_cycle(card, app, monday, sunday):
    ...
    fpath = os.path.join(utils.USERS_DIR,
                         getattr(app, "current_user", ""), "cycle.csv")
    cycle_row = None
    if os.path.exists(fpath):
        try:
            with open(fpath, "r", newline="", encoding="utf-8") as f:
    ...
    if produits_s and (cycle_actif or phase == "AVANT CYCLE"):
        ...
        for entry in entries:
            ...
            mk_label(l1, prod_name, size="small",
                     color=TH.TEXT).pack(side="left")
            if prod_dose:
                mk_badge(l1, prod_dose,
                         color=TH.ACCENT_DIM,
                         text_color=TH.ACCENT_GLOW).pack(side="left", padx=6)
            elif info.get("dose_min"):
                mk_badge(l1, f"réf: {info['dose_min']}",
                         color="#1a2a1a",
                         text_color=TH.TEXT_MUTED).pack(side="left", padx=6)
            if dang_str:
