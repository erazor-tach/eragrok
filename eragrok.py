...
try:
    from data import utils, entrainement_module, nutrition_module, cycle_module
...
class EragrokApp:
    ...
    def on_user_selected(self):
        ...
        if not info:
            messagebox.showerror("ERAGROK", "Profil introuvable."); return
        self.current_user       = info.get("prefix",
                                            sel.lower().replace(" ", "_"))
    ...
    def _build_sidebar(self):
        ...
        for icon, label, cmd in [
            ...
            ("📁", "Exporter CSV",   lambda: messagebox.showinfo(
                                         "Export", "Export CSV (placeholder)")),
        ]:
            mk_ghost_btn(
