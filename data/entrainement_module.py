...
DIFFICULTY_LEVEL_COLORS = {
    1: colors.HexColor("#e6fff2") if REPORTLAB_AVAILABLE else None,
    2: colors.HexColor("#d1fae5") if REPORTLAB_AVAILABLE else None,
    3: colors.HexColor("#fef3c7") if REPORTLAB_AVAILABLE else None,
    4: colors.HexColor("#fcd5c5") if REPORTLAB_AVAILABLE else None,
    5: colors.HexColor("#fee2e2") if REPORTLAB_AVAILABLE else None,
}
...
WEEKEND_OPTIONS = ["Off", "Pecs", "Cuisses", "Épaules", "Dos", "Bras"]
WEEKDAY_NAME_FR = {0: "Lundi", 1: "Mardi", 2: "Mercredi", 3: "Jeudi", 4: "Vendredi", 5: "Samedi", 6: "Dimanche"}
...
def show_entrainement_screen(app, program=None):
    ...
    # Programme label (menu Myofi / Sarco removed as requested)
    ...
    tk.Label(top, textvariable=app.program_selected_var, bg="#f3f4f6", font=("Helvetica", 10, "bold")).pack(side="left", padx=8)
    ...
    tk.Label(right, text="Détail", font=("Helvetica", 12, "bold"), bg="#ffffff").pack(anchor="w", padx=8, pady=(4, 4))
    ...
    # Week-end dropdowns for Saturday and Sunday
    weekend_frame = tk.Frame(gen_frame, bg="#ffffff")
    weekend_frame.pack(anchor="w", pady=6)
    tk.Label(weekend_frame, text="Samedi :", bg="#ffffff").grid(row=0, column=0, sticky="w", padx=(0,6))
    app.gen_saturday_var = tk.StringVar(value="Off")
    saturday_combo = ttk.Combobox(weekend_frame, values=WEEKEND_OPTIONS, textvariable=app.gen_saturday_var, state="readonly", width=12)
    saturday_combo.grid(row=0, column=1, sticky="w", padx=(0,12))
    tk.Label(weekend_frame, text="Dimanche :", bg="#ffffff").grid(row=0, column=2, sticky="w", padx=(0,6))
    app.gen_sunday_var = tk.StringVar(value="Off")
    sunday_combo = ttk.Combobox(weekend_frame, values=WEEKEND_OPTIONS, textvariable=app.gen_sunday_var, state="readonly", width=12)
    sunday_combo.grid(row=0, column=3, sticky="w")
...
def _on_tech_select(app):
    ...
    if tech:
        # Affiche la description pédagogique (si disponible) et les détails complets
        text_block = _get_beginner_text(tid, tech) + "\n\n---\n\nDétails techniques complets :\n"
        text_block += f"Nom: {tech.get('nom','—')}\nReps: {tech.get('reps','—')}\nCharge: {tech.get('charge','—')}\nRepos: {tech.get('repos','—')}\nObjectif: {tech.get('objectif','—')}\n\nNotes:\n{tech.get('notes','')}"
    else:
...
def _load_program(app, name):
    ...
    if name in ["Sarco", "Myofi"]:
        ...
        # default simple standard program
...
def _weekend_selection_for_day(app, weekday):
    """
    weekday: 5 = Saturday, 6 = Sunday
    Returns selection string or "Off"
    """
    if weekday == 5:
        return getattr(app, "gen_saturday_var", tk.StringVar(value="Off")).get()
    if weekday == 6:
        return getattr(app, "gen_sunday_var", tk.StringVar(value="Off")).get()
    return "Off"
    ...
    for week_idx, week in enumerate(month_weeks, start=1):
        ...
        for d in week:
            ...
            if weekday in day_to_group:
                ...
                # weekend handling using dropdowns; always show day name (Samedi/Dimanche)
                sel = _weekend_selection_for_day(app, weekday)
                day_name = WEEKDAY_NAME_FR.get(weekday, d.strftime("%A"))
                if sel == "Off":
                    app.program_listbox.insert(tk.END, f"{d.strftime(DATE_FORMAT)} - {day_name} : Off")
                else:
                    # use selected group name for the weekend day
                    if weekly_tech:
                        line = f"{d.strftime(DATE_FORMAT)} - {sel} : {weekly_tech['nom']} [{weekly_tech.get('reps','—')}] | {weekly_tech.get('charge','—')} ({weekly_tech['id']})"
                    else:
                        line = f"{d.strftime(DATE_FORMAT)} - {sel} : (technique manquante)"
                    app.program_listbox.insert(tk.END, line)
        ...
        for weekday in range(0, 7):
            ...
            if weekday in day_to_group:
                ...
                if sel == "Off":
                    ...
                    if tech:
                        line = f"{d.strftime(DATE_FORMAT)} - {sel} : {tech['nom']} [{tech.get('reps','—')}] | {tech.get('charge','—')} ({tech['id']})"
                    else:
                        line = f"{d.strftime(DATE_FORMAT)} - {sel} : (technique manquante)"
                    app.program_listbox.insert(tk.END, line)
...
def _open_tech_detail_dialog(app, tech):
    ...
    # Affiche la description pédagogique (si disponible) puis les notes complètes
    beginner_text = _get_beginner_text(tech.get("id"), tech)
    full_notes = tech.get("notes", "")
    txt.insert("1.0", beginner_text + "\n\n---\n\nDétails techniques complets :\n" + full_notes)
