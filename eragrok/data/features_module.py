...
def _embed_canvas(fig, master, app=None):
    """
    Intègre un figure matplotlib dans Tkinter de façon sécurisée.
    Bind <Destroy> pour annuler les after() du backend TkAgg
    (supprime les bgerror 'check_dpi_scaling' / 'update' à la navigation).
    """
    ...
    if app is not None:
        if not hasattr(app, "_mpl_canvases"):
            app._mpl_canvases = []
        app._mpl_canvases.append(canvas)

    def _on_destroy(event, c=canvas, f=fig):
        try:
            # Annuler les after() du backend TkAgg avant destruction
            tk_w = c.get_tk_widget()
            for attr in ("_idle", "_resize_callback"):
                try: tk_w.after_cancel(getattr(c, attr, ""))
                except Exception: pass
            # Forcer la déconnexion des event managers matplotlib
            try: c.mpl_disconnect(getattr(c, "_cidgcf", None))
            except Exception: pass
            try: plt.close(f)
            except Exception: pass
        except Exception:
...
def render_macro_pie(parent, cal, prot, gluc, lip):
    ...
    canvas, widget = _embed_canvas(fig, frame)
    widget.pack()
    return frame
...
def show_meal_generator(app):
    ...
    def _render_plan(frame, plan, n_meals, adj_lbl,
        ...
        mk_btn(pdf_row, "📄  PDF", _export_pdf,
               color="#1a1a2e", hover="#252540",
               width=90, height=TH.BTN_SM).pack(side="right")

        # Accepter + Historique → à gauche du PDF
        def _accept_plan_btn(p=plan, tc=tot_cal, tp=tot_prot, tg=tot_gluc, tl=tot_lip):
            lp = getattr(app, "_last_meal_plan", None)
            if lp: lp["accepted"] = True
            _save_plan(p=p, tc=tc, tp=tp, tg=tg, tl=tl)
            try:
                from data import nutrition_module as _nm
                _nm.show_nutrition_screen(app)
            except Exception: pass

        mk_btn(pdf_row, "✅  Accepter",  _accept_plan_btn,
               color=TH.SUCCESS, hover=TH.SUCCESS_HVR,
               width=130, height=TH.BTN_SM).pack(side="left", padx=(0,6))
        mk_btn(pdf_row, "💾  Historique", _save_plan,
               color=TH.GRAY, hover=TH.GRAY_HVR,
               width=120, height=TH.BTN_SM).pack(side="left")
