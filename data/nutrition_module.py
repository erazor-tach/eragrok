...
from data import weight_chart  # ← graphique poids
...
def render_dashboard(app):
    ...
    # fetch last weight — utilise get_user_dir
    poids_val = None
    ...
    if poids_val and age_val and sexe_val and objectif_val and taille_val:
        ...
        if nut:
            ...
            carb_pct     = nut.get('carb_pct', 0.45)
            fat_pct      = nut.get('fat_pct',  0.25)
    ...
    hg = tk.Frame(card, bg="#e0f2fe"); hg.pack(fill='both', expand=True, pady=(8, 0))

    metrics = tk.Frame(hg, bg="#e0f2fe")
    metrics.pack(side='left', padx=(6, 18), pady=6, anchor='n')

    def metric_row(parent, label, value, emoji=""):
        row = tk.Frame(parent, bg="#e0f2fe"); row.pack(anchor='w', pady=6)
    ...
    btns = tk.Frame(card_train, bg="#ffffff"); btns.pack(anchor='w')
    ttk.Button(btns, text="Ouvrir Entraînement",
               command=lambda: __import__('data.entrainement_module').entrainement_module.show_entrainement_screen(app)).pack(side='left', padx=6)
    ttk.Button(btns, text="Sarco",
               command=lambda: __import__('data.entrainement_module').entrainement_module.show_entrainement_screen(app, program="Sarco")).pack(side='left', padx=6)
    ttk.Button(btns, text="Myofi",
               command=lambda: __import__('data.entrainement_module').entrainement_module.show_entrainement_screen(app, program="Myofi")).pack(side='left', padx=6)
    ...
    ttk.Button(card_cycle, text="Ouvrir Cycle",
               command=lambda: __import__('data.cycle_module').cycle_module.show_cycle_disclaimer(app)).pack(anchor='w')
    ...
    btns2 = tk.Frame(card_short, bg="#ffffff"); btns2.pack(anchor='w', pady=8)
    ttk.Button(btns2, text="Nutrition",
               command=lambda: show_nutrition_screen(app)).pack(side='left', padx=6)
    ttk.Button(btns2, text="📈 Évolution poids",
               command=lambda: weight_chart.show_weight_chart(app)).pack(side='left', padx=6)  # ← NOUVEAU
    ttk.Button(btns2, text="Exporter CSV",
               command=lambda: messagebox.showinfo("Export", "Export CSV (placeholder)")).pack(side='left', padx=6)
...
def _get_last_entries(app, filename, n=1):
    if not utils.get_current_user_folder(app):
        return []
    path = os.path.join(utils.get_user_dir(app), filename)
...
def _draw_imc_gauge(parent_frame, imc_value, poids_actuel, taille_cm, bg_color="#e0f2fe"):
    ...
    try:
        ...
        def imc_to_angle(imc):
            min_imc, max_imc = 10.0, 60.0
            v = max(min_imc, min(max_imc, imc if imc is not None else min_imc))
            return math.radians(180 * ((v - min_imc) / (max_imc - min_imc)))

        for low, high, color in segments:
            a1, a2 = imc_to_angle(low), imc_to_angle(high)
            n      = 120
            thetas = [a1 + (a2 - a1) * i / n for i in range(n)]
            ax.bar(thetas, [6.6] * n, width=(a2 - a1) / n,
                   bottom=1.8, color=color, edgecolor=color, align='edge')

        for t in range(10, 61):
            ang = imc_to_angle(t)
            ax.plot([ang, ang], [7.2, 7.6], color='#0f172a', linewidth=0.5)
            if t % 5 == 0:
                ax.text(ang, 8.2, str(t), ha='center', va='center',
        ...
        if imc_value is not None:
            ...
            for low, high, cat in IMC_TABLE_5:
                if imc_value >= low and imc_value < high:
                    cat_label = cat
                    break
            ax.text(math.radians(90), 0.6,  cat_label,             ha='center', fontsize=12, fontweight='bold', color='#071133')
            ax.text(math.radians(90), -0.1, f"IMC {imc_value:.1f}", ha='center', fontsize=16, fontweight='bold', color='#071133')
            if poids_actuel and taille_cm:
                try:
                    ...
                    if poids_actuel < normal_min:
                        diff_text = f"+{normal_min - float(poids_actuel):.1f} kg → IMC 18.5"
                    elif poids_actuel > normal_max:
                        diff_text = f"-{float(poids_actuel) - normal_max:.1f} kg → IMC 25.0"
                    else:
                        diff_text = "Dans la zone normale"
                    ax.text(math.radians(90), -0.9, diff_text, ha='center', fontsize=9, color='#0f172a')
                except Exception:
                    pass
...
def show_nutrition_screen(app):
    for w in app.content.winfo_children():
        w.destroy()
    tk.Label(app.content, text=f"NUTRITION - Élève : {app.selected_user_name}",
             font=("Helvetica", 20, "bold"), bg="#f3f4f6", fg="#0f172a").pack(pady=12)

    top_frame = tk.Frame(app.content, bg="#f3f4f6"); top_frame.pack(fill='x', padx=20)
    cal_frame = tk.Frame(top_frame, bg="#f3f4f6"); cal_frame.pack(side='left', padx=8, pady=6)
    ...
    if not hasattr(app, "selection_mode"):
        app.selection_mode = "Single"

    ctrl_frame = tk.Frame(top_frame, bg="#f3f4f6"); ctrl_frame.pack(side='left', padx=12, pady=6, anchor='n')
    ...
    for m in modes:
        ttk.Radiobutton(ctrl_frame, text=m, value=m, variable=app.selection_mode_var,
                        command=lambda: _on_mode_change(app)).pack(anchor='w', pady=2)

    btns = tk.Frame(ctrl_frame, bg="#f3f4f6"); btns.pack(pady=6, anchor='w')
    ttk.Button(btns, text="Sélectionner la semaine",
               command=lambda: _select_week_from_calendar(app)).pack(side='left', padx=4)
    ttk.Button(btns, text="Sélectionner le mois",
               command=lambda: _select_month_from_calendar(app)).pack(side='left', padx=4)
    ttk.Button(btns, text="Effacer sélection",
               command=lambda: _clear_selection(app)).pack(side='left', padx=4)

    app.selected_summary_label = tk.Label(ctrl_frame, text="Aucune date sélectionnée",
                                           bg="#f3f4f6", justify='left', wraplength=260)
    app.selected_summary_label.pack(pady=(8, 0), anchor='w')
    app.calendar.bind("<<CalendarSelected>>", lambda ev: _on_calendar_select(app))

    # Formulaire calcul nutrition
    ...
    # dernier poids — utilise get_user_dir
    dernier_poids = ""
    user_dir    = utils.get_user_dir(app)
    fichier_nut = os.path.join(user_dir, "nutrition.csv") if user_dir else None
    if fichier_nut and os.path.exists(fichier_nut) and pd is not None:
    ...
    app.age_var.set(str(app.user_info.get('age', '') or '') if getattr(app, "user_info", None) else '')
    ...
    ttk.Combobox(mid, values=list(utils.ADJUSTMENTS.keys()),
                 textvariable=app.adjustment_var, state="readonly", width=36).grid(row=0, column=1, padx=6, pady=4)

    right = tk.Frame(calc_frame, bg="#ffffff"); right.pack(side='right', padx=8, pady=4, fill='x', expand=True)
    app.calories_label  = tk.Label(right, text="🔥 Calories : -",  bg="#ffffff", justify='left'); app.calories_label.pack(anchor='w', pady=2)
    app.proteines_label = tk.Label(right, text="🥩 Protéines : -", bg="#ffffff", justify='left'); app.proteines_label.pack(anchor='w', pady=2)
    app.glucides_label  = tk.Label(right, text="🍚 Glucides : -",  bg="#ffffff", justify='left'); app.glucides_label.pack(anchor='w', pady=2)
    app.lipides_label   = tk.Label(right, text="🥑 Lipides : -",   bg="#ffffff", justify='left'); app.lipides_label.pack(anchor='w', pady=2)
    ...
    ttk.Button(btn_frame, text="SAUVEGARDER NUTRITION (sur sélection)",
               command=lambda: _save_nutrition(app)).pack(side='left', padx=6)
    ttk.Button(btn_frame, text="📈 Graphique poids",
               command=lambda: weight_chart.show_weight_chart(app)).pack(side='left', padx=6)  # ← NOUVEAU
    ttk.Button(btn_frame, text="Retour Dashboard",
               command=lambda: render_dashboard(app)).pack(side='left', padx=6)

    # Historique
    hist_frame = tk.Frame(app.content); hist_frame.pack(padx=20, pady=8, fill='both', expand=True)
    tk.Label(hist_frame, text="Historique nutrition (filtré par sélection si applicable)",
             font=("Helvetica", 12, "bold")).pack(anchor='w')

    app.nutrition_tree = ttk.Treeview(hist_frame,
                                       columns=("Date", "Poids", "Calories", "Note"),
                                       show="headings", selectmode='browse')
    app.nutrition_tree.heading("Date",     text="Date")
    app.nutrition_tree.heading("Poids",    text="Poids (kg)")
    app.nutrition_tree.heading("Calories", text="Calories")
    app.nutrition_tree.heading("Note",     text="Note")
    ...
    action_frame = tk.Frame(hist_frame); action_frame.pack(anchor='w', pady=(6, 0))
    ttk.Button(action_frame, text="Modifier sélection",
               command=lambda: _edit_selected(app)).pack(side='left', padx=6)
    ttk.Button(action_frame, text="Supprimer sélection",
               command=lambda: _delete_selected(app)).pack(side='left', padx=6)
...
def _on_calendar_select(app):
    ...
    if mode == "Single":
        ...
        start, _ = _week_range_for_date(d)
        app.selected_dates = {start + datetime.timedelta(days=i) for i in range(7)}
    elif mode == "Month":
        first, last = _month_range_for_date(d)
        app.selected_dates = {first + datetime.timedelta(days=i) for i in range((last - first).days + 1)}
    elif mode == "Multiple":
        if not hasattr(app, "selected_dates"):
            app.selected_dates = set()
...
def _render_selected_summary(app):
    ...
    app.selected_summary_label.config(text=f"{len(sorted_dates)} date(s) : {preview}{more}")
...
def _refresh_history(app, fichier_nut):
    ...
    try:
        ...
        if pd is not None:
            ...
            df = df.sort_values('__date').reset_index(drop=False)
            cols = list(df.columns)
            for _, r in df.iterrows():
                file_index = int(r['index'])
                date_obj   = r['__date']
            ...
            for idx, r in enumerate(reader[1:], start=1):
                date_obj = _try_parse_date(r[0] if len(r) > 0 else "")
                if date_obj is None:
                    continue
                rows.append((idx, date_obj, date_obj.strftime("%d/%m/%Y"),
                             r[1] if len(r) > 1 else "",
                             r[3] if len(r) > 3 else "",
                             r[7] if len(r) > 7 else ""))
        if not rows:
        ...
        sel = getattr(app, "selected_dates", set())
        if sel:
            filtered = sorted([r for r in rows if r[1] in sel], key=lambda x: x[1])
            for r in filtered:
                tree.insert("", "end", iid=str(r[0]), values=(r[2], r[3], r[4], r[5]))
        else:
            for r in sorted(rows, key=lambda x: x[1])[-30:]:
                tree.insert("", "end", iid=str(r[0]), values=(r[2], r[3], r[4], r[5]))
    except Exception:
        try:
            with open(fichier_nut, 'r', newline='', encoding='utf-8') as f:
...
def _edit_selected(app):
    ...
    _open_edit_dialog(app, iid, vals[0], vals[1], vals[2], vals[3])
...
def _open_edit_dialog(app, iid, date_str, poids, calories, note):
    ...
    bf = tk.Frame(dlg); bf.grid(row=4, column=0, columnspan=2, pady=8)
    ttk.Button(bf, text="Enregistrer",
               command=lambda: _save_edit(app, dlg, iid, date_var.get().strip(),
                                          poids_var.get().strip(), cal_var.get().strip(),
                                          note_text.get("1.0", tk.END).strip())).pack(side='left', padx=6)
    ttk.Button(bf, text="Annuler", command=dlg.destroy).pack(side='left', padx=6)
    ...
    try:
        if pd is not None:
            ...
            try:
                ...
                messagebox.showerror("Erreur", "Impossible d'identifier la ligne.")
                return
            if idx not in df.index:
                ...
                idx = matches.index[0]
            cols = list(df.columns)
            df.at[idx, cols[0]] = d.strftime("%d/%m/%Y")
            for c in cols:
                if 'poids'  in str(c).lower(): df.at[idx, c] = poids;    break
            for c in cols:
                if 'calor'  in str(c).lower(): df.at[idx, c] = calories;  break
            for c in cols:
                if 'note'   in str(c).lower(): df.at[idx, c] = note;      break
            ...
            if idx < 0 or idx >= len(data):
                for j, row in enumerate(data):
                    if _try_parse_date(row[0] if row else "") == d:
                        idx = j; break
                else:
                    messagebox.showerror("Erreur", "Ligne introuvable.")
                    return
            ...
            row[0] = d.strftime("%d/%m/%Y"); row[1] = poids; row[3] = calories; row[7] = note
            data[idx] = row
...
def _delete_selected(app):
    iid = _get_selected_item_iid(app)
    if not iid:
        messagebox.showinfo("Supprimer", "Sélectionne d'abord une ligne.")
        return
...
def _update_calc(app):
    try:
        ...
        if not poids or not age or not getattr(app, "user_info", None):
            return
        ...
        if getattr(app, "calories_label", None):
            app.calories_label.config(
                text=f"🔥 Calories : {adjusted_cal:.0f} kcal\n"
                     f"(TDEE: {nut['tdee']:.0f} | Ajust: {adj_frac*100:+.1f}%)")
...
def _save_nutrition(app):
    ...
    if not sel:
        try:
            ...
            d = _try_parse_date(app.calendar.get_date())
        sel = {d or datetime.date.today()}

    sel_dates = set()
    for s in sel:
        d = s if isinstance(s, datetime.date) else _try_parse_date(str(s))
        if d:
            sel_dates.add(d)
    if not sel_dates:
    ...
    poids    = app.poids_var.get().strip()
    age      = app.age_var.get().strip()
    calories = str(app._last_computed_calories) if getattr(app, '_last_computed_calories', None) else ""
