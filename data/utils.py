...
USERS_FILE = str(_ROOT / "users" / "eragrok_users.csv")  # conservé pour compat
...
def get_user_info(display_name: str):
    ...
    return {
        "name": row["name"], "age": _int(row["age"]), "sexe": row["sexe"],
...
def add_user(name, age, sexe, taille, poids="", objectif="", ajustement="Maintien (0%)"):
    ...
    con.execute(
        "INSERT INTO users (name,age,sexe,taille,poids,objectif,ajustement) VALUES (?,?,?,?,?,?,?)",
        (name, str(age), sexe, str(taille), str(poids), objectif, ajustement))
...
def update_user(old_name, new_name, age, sexe, taille, poids, objectif, ajustement):
    ...
    con.execute(
        "UPDATE users SET name=?,age=?,sexe=?,taille=?,poids=?,objectif=?,ajustement=? WHERE name=?",
        (new_name, str(age), sexe, str(taille), str(poids), objectif, ajustement, old_name))
    con.commit(); con.close()
