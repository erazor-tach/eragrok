...
def force_reload_prices(app, on_done=None) -> None:
    ...
    try:
        ...
        con.commit()
        con.close()
        log.info("food_prices purgée (prix manuels conservés) — rechargement des défauts")

        # 2. Recharger les défauts
        load_defaults(app)
        _db.settings_set(app, "prices_version", str(PRICES_VERSION))
        n_defaults = len(DEFAULT_PRICES)
        log.info("%d prix par défaut rechargés", n_defaults)

        # 3. Lancer MAJ OFF en background
        def _off_done(n_up, n_fail):
            log.info("MAJ OFF post-reload : %d mis à jour, %d inchangés", n_up, n_fail)
            if on_done:
                try:
        ...
        log.warning("force_reload_prices: %s", e)
        if on_done:
            try:
                on_done(0, 0)
            except Exception:
