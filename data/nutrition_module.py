...
def _refresh_tree(app, fichier):
    ...
    try:
        ...
        for _rowdict in rows_data:
            row = [_rowdict.get("date",""), _rowdict.get("poids",""), _rowdict.get("age",""),
                   _rowdict.get("calories",""), _rowdict.get("proteines",""),
                   _rowdict.get("glucides",""), _rowdict.get("lipides",""), _rowdict.get("note","")]
...
def _delete_selected(app):
    ...
    try:
        _db.nutrition_delete_by_date(app, d)
        _refresh_tree(app, "")
    except Exception as e: messagebox.showerror("ERAGROK", str(e))
...
def _render_cycle_card(parent, app):
    ...
    if not cycle:
        ...
        if cycle["produits"]:
            ...
            # Parsing : "Produit: dose | Produit2: dose2 | ..."
            entries = [p.strip() for p in cycle["produits"].split("|") if p.strip()]

            # Score de dangerosité globale
            dang_scores = []
            ...
            for entry in entries:
                ...
                # Ligne principale : nom + dose
                top = ctk.CTkFrame(row, fg_color="transparent")
                top.pack(fill="x", padx=10, pady=(6, 2))
                mk_label(top, f"• {prod_name}", size="small",
                ...
                if info:
                    ...
                    # Ligne dose référence + dangerosité
                    ...
                    # Notes risques (ligne séparée si présentes)
                    if notes_p:
            ...
            if dang_scores:
                ...
                risk_txt = {0:"—", 1:"✅ Faible", 2:"✅ Modéré",
                            3:"⚠️  Élevé", 4:"🔴 Très élevé",
                            5:"🔴 Extrême"}.get(max_dang, "—")
                ...
                mk_label(rr, risk_txt, size="body",
                         color=risk_col).pack(side="left")

        # ── Note utilisateur ──────────────────────────────────────────────────
        if cycle.get("note"):
            note_f = ctk.CTkFrame(c, fg_color=TH.BG_CARD2, corner_radius=6)
            note_f.pack(fill="x", padx=16, pady=(4, 10))
            mk_label(note_f, f"📝  {cycle['note'][:120]}{'…' if len(cycle['note'])>120 else ''}",
...
def render_dashboard(app):
    ...
    # ligne inputs — centrée
    ir_wrap = ctk.CTkFrame(nc, fg_color="transparent")
    ir_wrap.pack(anchor="center", pady=(0, 10))
    ir = ctk.CTkFrame(ir_wrap, fg_color="transparent")
    ir.pack(anchor="center")
    # Poids seulement — âge calculé automatiquement depuis date_naissance
    cc_p = ctk.CTkFrame(ir, fg_color="transparent")
    cc_p.pack(side="left", padx=(0, 16))
    mk_label(cc_p, "Poids (kg)", size="small", color=TH.TEXT_SUB).pack(anchor="center", pady=(0, 4))
    ...
    # ── Résultats macros (4 valeurs sans encadrement, centrées) ──────────────
    ...
    # Frame camembert macros (rempli dynamiquement par _update_calc)
    ...
    hg.pack(fill="x", padx=16, pady=(4, 14))

    # Panneau métriques (gauche)
    ...
    if _feat:
        try:
            _feat.render_weight_chart(scroll, app)
        except Exception:
            pass
    ...
    if _feat:
        try:
            _feat.render_cycle_timeline(scroll, app)
        except Exception:
            pass

    # ── Plan alimentaire — résumé ou bouton si aucun ────────────────────────
    if _feat:
        plan_card = mk_card(scroll)
        plan_card.pack(fill="x", padx=0, pady=(0, 14))
        mk_title(plan_card, "  🍽  PLAN ALIMENTAIRE DU JOUR").pack(
            anchor="w", padx=16, pady=(14,6))
        ...
        if last_plan and last_plan.get("plan"):
            plan_data = last_plan["plan"]
            tot_cal  = sum(m["tot_cal"] for m in plan_data)
            tot_prot = sum(m["tot_p"]   for m in plan_data)
            tot_gluc = sum(m["tot_g"]   for m in plan_data)
            tot_lip  = sum(m["tot_l"]   for m in plan_data)
            # Ligne macro résumée
            mac_row = ctk.CTkFrame(plan_card, fg_color=TH.BG_CARD2, corner_radius=6)
            mac_row.pack(fill="x", padx=16, pady=(0,8))
            for lbl, val, col in [
                ("🔥", f"{tot_cal:.0f} kcal", TH.ACCENT_GLOW),
                ("🥩", f"{tot_prot:.0f}g P",  "#4aaa4a"),
                ("🍚", f"{tot_gluc:.0f}g G",  "#3b82f6"),
                ("🥑", f"{tot_lip:.0f}g L",   "#a855f7"),
            ]:
                cf = ctk.CTkFrame(mac_row, fg_color="transparent")
                cf.pack(side="left", expand=True, pady=6)
                mk_label(cf, lbl, size="small", color=TH.TEXT_MUTED).pack()
                mk_label(cf, val, size="body",  color=col).pack()
            # Repas résumés
            for meal in plan_data:
                mr = ctk.CTkFrame(plan_card, fg_color="transparent")
                mr.pack(fill="x", padx=16, pady=1)
                mk_label(mr, meal["name"], size="small",
                         color=TH.TEXT).pack(side="left")
                foods_str = " · ".join(i["food"].split(" (")[0] for i in meal["items"][:3])
                if len(meal["items"]) > 3:
                    foods_str += f" +{len(meal['items'])-3}"
                mk_label(mr, foods_str, size="small",
                         color=TH.TEXT_MUTED).pack(side="right")
            # Bouton modifier
            pb = ctk.CTkFrame(plan_card, fg_color="transparent")
            pb.pack(fill="x", padx=16, pady=(8,12))
            mk_btn(pb, "✏  Modifier le plan",
                   lambda: _feat.show_meal_generator(app),
                   color=TH.GRAY, hover=TH.GRAY_HVR,
                   width=160, height=TH.BTN_SM).pack(side="right")
        else:
            # Aucun plan — juste le bouton
            pr = ctk.CTkFrame(plan_card, fg_color="transparent")
            pr.pack(fill="x", padx=16, pady=12)
            mk_label(pr, "Aucun plan généré pour aujourd'hui.",
    ...
    # ── Section Semaine en cours & Semaine suivante ────────────────────────────
...
def _render_week_view(app, parent):
    ...
    for col_idx, (monday, label) in enumerate([
        ...
        # ── Cycle ──
        _render_week_cycle(card, app, monday, sunday)
...
def show_nutrition_screen(app):
    ...
    app.sel_label.pack(anchor="w", padx=14, pady=(6, 8))

    mk_sep(left).pack(fill="x", padx=14, pady=(0, 8))
    mk_label(left, "Export", size="small",
             color=TH.TEXT_MUTED).pack(anchor="w", padx=14, pady=(0, 4))

    def _export_nutr_pdf():
        try:
            from data import pdf_utils
            pdf_utils.export_nutrition_pdf(app, ask_path=True)
        except Exception as e:
            from tkinter import messagebox as _mb
            _mb.showerror("ERAGROK", f"Erreur PDF : {e}")

    def _export_plan_pdf():
        try:
            last = getattr(app, "_last_meal_plan", None)
            if not last or not last.get("plan"):
                from tkinter import messagebox as _mb
                _mb.showwarning("ERAGROK", "Génère d'abord un plan alimentaire.")
                return
            from data import pdf_utils
            pdf_utils.export_meal_plan_pdf(app, last["plan"], ask_path=True)
        except Exception as e:
            from tkinter import messagebox as _mb
            _mb.showerror("ERAGROK", f"Erreur PDF : {e}")

    mk_btn(left, "📄  PDF Historique nutrition", _export_nutr_pdf,
           color="#1a1a2e", hover="#252540",
           width=200, height=TH.BTN_SM).pack(padx=14, pady=(0, 4))
    mk_btn(left, "📄  PDF Plan alimentaire", _export_plan_pdf,
           color="#1a1a2e", hover="#252540",
           width=200, height=TH.BTN_SM).pack(padx=14, pady=(0, 14))
        ...
        def _open_meal_calc_screen():
            def _apply(cal, prot, gluc, lip):
                try:
                    ...
                    p   = app.poids_var.get().strip() or str(ui.get("poids",""))
                    ...
                    pass
            _feat.show_meal_calculator(app, on_apply=_apply)
        mk_btn(save_row, "🍽  Calculateur repas", _open_meal_calc_screen,
               color="#1a2a3a", hover="#223344",
               width=180, height=TH.BTN_LG).pack(side="right")
        mk_btn(save_row, "📋  Plan alimentaire",
               lambda: _feat.show_meal_generator(app),
               color="#0e2a1a", hover="#14381f",
               width=180, height=TH.BTN_LG).pack(side="right", padx=(0, 6))
